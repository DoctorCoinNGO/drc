...
pkgdatadir = $(datadir)/drccore
pkgincludedir = $(includedir)/drccore
pkglibdir = $(libdir)/drccore
pkglibexecdir = $(libexecdir)/drccore
...
CONFIG_HEADER = $(top_builddir)/src/config/drc-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/drc-config.h.in \
...
ACLOCAL = ${SHELL} /mnt/c/tmp/drc/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
ARFLAGS = cr
AUTOCONF = ${SHELL} /mnt/c/tmp/drc/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /mnt/c/tmp/drc/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /mnt/c/tmp/drc/build-aux/missing automake-1.15
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = drc-cli
BITCOIN_DAEMON_NAME = drcd
BITCOIN_GUI_NAME = drc-qt
BITCOIN_TX_NAME = drc-tx
...
COPYRIGHT_HOLDERS_FINAL = The DRC Core developers
COPYRIGHT_HOLDERS_SUBSTITUTION = DRC Core
COPYRIGHT_YEAR = 2018
CPP = gcc -E
...
MAINT = 
MAKEINFO = ${SHELL} /mnt/c/tmp/drc/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
MINIUPNPC_LIBS = 
...
OTOOL64 = 
PACKAGE = drccore
PACKAGE_BUGREPORT = https://github.com/NicholasAdmin/DRC/issues
PACKAGE_NAME = DRC Core
PACKAGE_STRING = DRC Core 1.2.0
PACKAGE_TARNAME = drccore
PACKAGE_URL = 
...
PYTHONPATH = /home/test/drc/depends/x86_64-w64-mingw32/share/../native/lib/python/dist-packages:
QR_CFLAGS = 
QR_LIBS = 
QT4_CFLAGS = 
QT4_LIBS = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /mnt/c/tmp/drc
abs_srcdir = /mnt/c/tmp/drc
abs_top_builddir = /mnt/c/tmp/drc
abs_top_srcdir = /mnt/c/tmp/drc
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /mnt/c/tmp/drc/build-aux/install-sh
...
BITCOIND_BIN = $(top_builddir)/src/drcd$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/drc-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/drc-cli$(EXEEXT)
BITCOIN_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
OSX_APP = DRC-Qt.app
OSX_DMG = DRC-Core.dmg
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_drc_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_drc_coverage.info test_drc.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/DRC-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/drc-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/drc-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/drc-config.h
$(top_srcdir)/src/config/drc-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/drc-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/DRC-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/DRC-Qt

$(OSX_DMG): $(APP_DIST_EXTRAS)
	$(GENISOIMAGE) -no-cache-inodes -D -l -probe -V "DRC-Core" -no-pad -r -dir-mode 0755 -apple -o $@ dist
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/DRC-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_drc.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_drc -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_drc_filtered.info: test_drc.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_drc_filtered.info
#	$(MKDIR_P) qa/tmp
...
#rpc_test.info: test_drc_filtered.info
...
#test_drc_coverage.info: baseline_filtered_combined.info test_drc_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_drc_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_drc_filtered.info block_test_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_drc_filtered.info -a block_test_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_drc.coverage/.dirstamp:  test_drc_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_drc.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf coverage_percent.txt test_drc.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
