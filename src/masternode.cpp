...
void CMasternode::Check(bool fForce)
{
    ...
    LogPrint("masternode", "CMasternode::Check -- mn lastPing.sigTime = %d  and sigTime = %d \n", lastPing.sigTime, sigTime);
    if(lastPing.sigTime - sigTime < MASTERNODE_MIN_MNP_SECONDS) { // 10 * 60
...
bool CMasternodeBroadcast::Create(CTxIn txin, CService service, CKey keyCollateralAddressNew, CPubKey pubKeyCollateralAddressNew, CKey keyMasternodeNew, CPubKey pubKeyMasternodeNew, std::string &strErrorRet, CMasternodeBroadcast &mnbRet)
{
    ...
    LogPrintf("CMasternodeBroadcast::create before CMasternodeBroadcast .....%s", "ok");
    if (fImporting || fReindex) return false;

    LogPrintf("CMasternodeBroadcast::enter in and start to create .....%s", "ok");
...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
...

    {
        ...
        if(!lockMain) {
            ...
            LogPrint("masternode", "CMastern::odeBroadcast::CheckOutpoint -- Failed to aquire lock, addr=%s", addr.ToString());
        ...
        if(!pcoinsTip->GetCoins(vin.prevout.hash, coins) ||
	     ...
	     CAmount collat_required  = 1000 * COIN;
        if(coins.vout[vin.prevout.n].nValue != collat_required) {
		    if (
            LogPrint("masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have 1000 DRC, masternode=%s\n", vin.prevout.ToStringShort()));
            return false;
        }
    ...
    // should be at least not earlier than block when 100 DRC tx got nMasternodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex* pMNIndex = (*mi).second; // block for 100 DRC tx -> 1 confirmation
...
bool CMasternodeBroadcast::Sign(CKey& keyCollateralAddress)
{
    ...
    LogPrintf("CMasternodeBroadcast::CMasternodePing -- sigTime = %d \n", sigTime);
...
CMasternodePing::CMasternodePing(CTxIn& vinNew)
{
    ...
    LogPrintf("CMasternodePing::CMasternodePing -- sigTime = %d \n", sigTime);
...
bool CMasternodePing::Sign(CKey& keyMasternode, CPubKey& pubKeyMasternode)
{
    ...
    LogPrintf("CMasternodePing::Sign -- sigTime = %d \n", sigTime);
...
bool CMasternodePing::CheckAndUpdate(CMasternode* pmn, bool fFromNewBroadcast, int& nDos)
{
    ...
    if (!pmn->IsEnabled()){
        LogPrint("masternode", "CMasternodePing::CheckAndUpdate -- Masternode ping accepted, masternode=%s  but pm is not enable!!!!!  \n", vin.prevout.ToStringShort());
        return false;
    }
