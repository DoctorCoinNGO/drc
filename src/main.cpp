...
#if defined(NDEBUG)
# error "DRC Core cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "DRCCoin Signed Message:\n";
...
bool IsInitialBlockDownload()
{
    ...
    if (fImporting || fReindex){
        LogPrintf("IsInitialBlockDownload: fImporting || fReindex => true\n");
        return true;
    }
    LOCK(cs_main);
    const CChainParams& chainParams = Params();
    if (fCheckpointsEnabled && chainActive.Height() < Checkpoints::GetTotalBlocksEstimate(chainParams.Checkpoints())){
        LogPrintf("IsInitialBlockDownload: chainActive.Height() < Checkpoints::GetTotalBlocksEstimate(chainParams.Checkpoints()) => true\n");
        return true;
    }
    // bool state = (chainActive.Height() < pindexBestHeader->nHeight - 24 * 6 ||
    bool state = (chainActive.Height() < pindexBestHeader->nHeight - 10 ||
            std::max(chainActive.Tip()->GetBlockTime(), pindexBestHeader->GetBlockTime()) < GetTime() - chainParams.MaxTipAge());

    LogPrintf("%s: chainActive.Height() = %d(pindexBestHeader->nHeight - 24 * 6) = %d \n", __func__, chainActive.Height(), (pindexBestHeader->nHeight - 24 * 6));

    // if(pindexBestHeader->nHeight == 0) {
    //   state = false;
    // }

    if (!state){
        LogPrintf("IsInitialBlockDownload: !state => true\n");
        lockIBDState = true;
    }
    LogPrintf("IsInitialBlockDownload: return state \n");
    return state;
}
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // DRC : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(DRC): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(DRC): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // DRC : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(DRC): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(DRC): spork is off, skipping transaction locking checks\n");
    }
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /*
        DRC Related Inventory Messages
