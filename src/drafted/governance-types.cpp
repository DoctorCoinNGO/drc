...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "DRCNetwork") type = DRCNetwork;
    if(strType == "DRCNetworkVariable") type = DRCNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == DRCNetwork) s = "DRCNetwork";
    if(type == DRCNetworkVariable) s = "DRCNetworkVariable";
