...
const char * const BITCOIN_CONF_FILENAME = "drc.conf";
const char * const BITCOIN_PID_FILENAME = "drcd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "drc" is a composite category enabling all DRC-related debug output
            if(ptrCategory->count(string("drc"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DRCCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DRCCore
    // Mac: ~/Library/Application Support/DRCCore
    // Unix: ~/.drccore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DRCCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/DRCCore";
#else
    // Unix
    return pathRet / ".drccore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty drc.conf if it does not excist
        ...
        if (configFile != NULL) {
        	...
        	LogPrintf("drc.conf file not found or can't be created\n");
    ...
    {
        // Don't overwrite existing settings so command line settings override drc.conf
...
void AddSeedsToConfigFile(FILE* configFile) {
    ...
    //  fprintf(configFile,"addnode=144.202.109.173\r\n");
    //  fprintf(configFile,"addnode=144.202.109.174\r\n");
    //  fprintf(configFile,"addnode=144.202.109.175\r\n");
}
