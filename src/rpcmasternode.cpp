...
UniValue masternode(const UniValue& params, bool fHelp)
{
        ...
        (strCommand != "start" && strCommand != "start-alias" && strCommand != "start-all" && strCommand != "start-missing" &&
         ...
         strCommand != "connect" && strCommand != "outputs" && strCommand != "status"))
            throw std::runtime_error(
                ...
                "  start        - Start local Hot masternode configured in drc.conf\n"
    ...
    if (strCommand == "start-alias")
    {
        ...
        LogPrintf("CRpcmasternode:: start-alias in here .... 1  \n");
        ...
        LogPrintf("CRpcmasternode:: start-alias in here .... 2  \n");

        BOOST_FOREACH(CMasternodeConfig::CMasternodeEntry mne, masternodeConfig.getEntries()) {
            if(mne.getAlias() == strAlias) {
                LogPrintf("CRpcmasternode:: start-alias in here .... 3  \n");

                fFound = true;
                std::string strError;
                CMasternodeBroadcast mnb;

                LogPrintf("CRpcmasternode:: before invoke CMasternodeBroadcast. .....%s", "ok");

                bool fResult = CMasternodeBroadcast::Create(mne.getIp(), mne.getPrivKey(), mne.getTxHash(), mne.getOutputIndex(), strError, mnb);

                LogPrintf("CRpcmasternode:: start-alias in here .... 4  \n");

                statusObj.push_back(Pair("result", fResult ? "successful" : "failed"));
                if(fResult) {
                    LogPrintf("CRpcmasternode:: start-alias in here .... 5  \n");

                    mnodeman.UpdateMasternodeList(mnb);
                    mnb.Relay();
                } else {
                    LogPrintf("CRpcmasternode:: start-alias in here .... 6  \n");

                    statusObj.push_back(Pair("errorMessage", strError));
                }
                LogPrintf("CRpcmasternode:: start-alias in here .... 7  \n");
...
UniValue masternodelist(const UniValue& params, bool fHelp)
{
                ...
                strMode != "protocol" && strMode != "payee" && strMode != "rank" && strMode != "status"))
    {
        throw std::runtime_error(
                ...
                "  payee          - Print DRC address associated with a masternode (can be additionally filtered,\n"
                "                   partial match)\n"
