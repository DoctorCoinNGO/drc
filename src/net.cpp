...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (DRC users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "DRC Core " + FormatFullVersion();
...
void static ProcessOneShot()
{
    ...
    if (grant) {
        LogPrintf("ProcessOneShot: invoke OpenNetworkConnection...");
...
void ThreadOpenConnections()
{
    ...
    if (mapArgs.count("-connect") && mapMultiArgs["-connect"].size() > 0)
    {
        for (int64_t nLoop = 0;; nLoop++)
        {
            ...
            {
                CAddress addr;
                LogPrintf("ThreadOpenConnections: invoke OpenNetworkConnection but pszDest == null \n");
    ...
    while (true)
    {
        ...
        if (addrConnect.IsValid()){
            LogPrintf("ThreadOpenConnections: addrConnect.IsValid() invoke OpenNetworkConnection....\n");
...
void ThreadOpenAddedConnections()
{
    ...
    for (unsigned int i = 0; true; i++)
    {
        ...
        {
            CSemaphoreGrant grant(*semOutbound);
            LogPrintf("ThreadOpenAddedConnections: invoke OpenNetworkConnection...\n");
...
bool BindListenPort(const CService &addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. DRC Core is probably already running."), addrBind.ToString());
        else
